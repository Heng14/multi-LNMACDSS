...
class SurvivalModel:
	...
	def fit(self, datasets_train, datasets_val, datasets_test, train_name=None, val_name=None, test_name=None, loss_func='hinge', epochs=500, lr=0.001, mode='merge', batch_size = 8):
		...
		Args
			datasets_train:     training datasets, a list of (X, time_value, event) tuples
			datasets_val:       validation datasets, a list of (X, time_value, event) tuples
	...
	def __build_graph(self):
		...
		with tf.name_scope('metric'):
			...
			if self.loss_func=='hinge':
				...
				loss = self.__cox_loss_yh(score, time_value, event)
	...
	def __train_decentralize(self):
		...
		for epoch in range(self.epochs):
			for X_batch, time_value_batch, event_batch in self.datasets_train:
	...
	def __train_merge(self):
		...
		#X_train, time_value_train, event_train = utils.get_datasets(self.datasets_train)
		...
		for epoch in range(self.epochs):
			...
			for X_batch, Y_batch in datagen.flow(X_train, time_value_event, batch_size=self.batch_size):
				...
				self.__sess.run(self.__train_op, feed_dict={self.__X: X_batch, self.__time_value: time_value_batch, self.__event: event_batch, K.learning_phase(): 1})
				#print (self.__sess.run([self.__train_op, self.__loss], feed_dict={self.__X: X_batch, self.__time_value: time_value_batch, self.__event: event_batch, K.learning_phase(): 1}))
			#print (self.__sess.run([self.__score, self.__loss, self.__ci], feed_dict={self.__X: X_batch, self.__time_value: time_value_batch, self.__event: event_batch, K.learning_phase(): 0}))
	...
	def __cox_loss(self, score, time_value, event):
		'''
		Args
			score: 		predicted s0.6891678082852567urvival time_value, tf tensor of shape (None, 1)
			time_value:		true survival time_value, tf tensor of shape (None, )
			event:		event, tf tensor of shape (None, )
		Return
	...
	def __cox_loss_yh(self, score, time_value, event):
		'''
		Args
			score: 		predicted survival time_value, tf tensor of shape (None, 1)
			time_value:		true survival time_value, tf tensor of shape (None, )
	...
	def __print_loss_ci(self):
		...
		for i, (X_batch, time_value_batch, event_batch) in enumerate(self.datasets_train):
			loss_train[i], ci_train[i] = self.evaluate(X_batch, time_value_batch, event_batch)

		## losses and c-indices on validation
		loss_val = np.zeros(len(self.datasets_val))
		ci_val = np.zeros(len(self.datasets_val))
		for i, (X_batch, time_value_batch, event_batch) in enumerate(self.datasets_val):
			loss_val[i], ci_val[i] = self.evaluate(X_batch, time_value_batch, event_batch)
