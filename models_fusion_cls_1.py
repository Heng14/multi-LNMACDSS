"""
this is for classification metastasis or not and the network output only one scoret
"""
import os
...
class SurvivalModel:
	...
	def __build_graph(self):
		...
		with tf.name_scope('metric'):
			acc = self.__calc_acc(score, Y)
			if self.loss_func=='cross_entropy':
				#loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=score, labels=Y))
				#loss = tf.reduce_mean(tf.nn.l2_loss(score-Y))
				loss = tf.nn.l2_loss(score-Y)
	...
	def __calc_acc(self, score, Y):
		...
		correct_prediction = tf.equal(tf.greater(score, 0.5), tf.cast(Y, tf.bool))
		acc = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
	...
	def __train_merge(self):
		...
		for epoch in range(self.epochs):
			...
			for X_batch, Y_batch in datagen.flow(X_train, Y_train, batch_size=self.batch_size):
				...
				#print (self.__sess.run([self.__train_op, self.__score, self.__Y, self.__loss], feed_dict={self.__X: X_batch, self.__Y: Y_batch, K.learning_phase(): 1}))
	...
	def __print_loss_acc_yh(self, datasets):
		...
		for i in range(len(X)//bs):
			...
			score_i, Y_i = self.__sess.run([self.__score, self.__Y], feed_dict = {self.__X: X_batch, self.__Y: Y_batch, K.learning_phase(): 0})
			#print (score_i)
			if i == 0:
				socre_all = score_i
				Y_all = Y_i
