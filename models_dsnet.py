...
class SurvivalModel:
	...
	def __build_graph(self):
		...
		with tf.name_scope('input'):
			...
			X1 = tf.placeholder(dtype=tf.float32, shape=(None, )+input_shape, name='X1')
		...
		with tf.name_scope('output'):
			...
			score = tf.identity(self.model([X, X1]), name='score')
	...
	def __train_merge(self):
		...
		for epoch in range(self.epochs):
			...
			batches = 0
			for X_batch_merge, Y_batch in datagen.flow(X_merge, time_event, batch_size=self.batch_size):
				batches += 1
				...
				self.__sess.run(self.__train_op, feed_dict={self.__X: X_batch, self.__X1: X_batch1, self.__time: time_batch, self.__event: event_batch, K.learning_phase(): 1})
			...
			if epoch%2==0:
				...
				self.__print_loss_ci_yh_dsnet(self.datasets_val, self.datasets_val_1)
				print (f'epoch {epoch} test log:')
				self.__print_loss_ci_yh_dsnet(self.datasets_test, self.datasets_test_1)
	...
	def __print_loss_ci_yh_dsnet(self, datasets, datasets_1):
		'''
		...
		for i in range(len(X)//bs):
			X_batch, X_batch1, time_batch, event_batch = X[bs*i:bs*(i+1),...], X_1[bs*i:bs*(i+1),...], time[bs*i:bs*(i+1),...], event[bs*i:bs*(i+1),...]

			score_i, time_i, event_i = self.__sess.run([self.__score, self.__time, self.__event], feed_dict = {self.__X: X_batch, self.__X1: X_batch1, self.__time: time_batch, self.__event: event_batch, K.learning_phase(): 0})
		...
		i += 1
		X_batch, X_batch1, time_batch, event_batch = X[bs*i:len(X),...], X_1[bs*i:bs*(i+1),...], time[bs*i:len(time),...], event[bs*i:len(event),...]
		#loss_i, ci_i = self.evaluate(X_batch, time_batch, event_batch)
